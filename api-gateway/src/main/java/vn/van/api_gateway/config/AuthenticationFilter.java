...
import vn.van.api_gateway.service.IdentityService;
...
public class AuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String token = authHeader.getFirst().startsWith("Bearer ")
        ...
        identityService.introspect(token).subscribe(introspectResponseApiResponse -> {
            log.info("valid: {}", introspectResponseApiResponse.getData().isValid());
        });
        return identityService.introspect(token).flatMap(
                introspectResponse ->
                        introspectResponse.getData().isValid()
    ...
    private Mono<Void> unauthenticated(ServerHttpResponse response) {
        String body = null;
        try {
            body = objectMapper.writeValueAsString(ApiResponse.builder()
